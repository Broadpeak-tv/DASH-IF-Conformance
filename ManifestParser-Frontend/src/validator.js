function Validator({ modules }) {
  ...
  async function handleValidation() {
    ...
    UI.getElement('log-output').value = ''
    ...
    switch (_state.activeMpdForm) {
      case URL_TYPE: {
        ...
        try {
          result = await MpdParsingService.validateContentByUrl({
            mpdUrl,
            activeModules,
          });
        } catch (error) {
      ...
      case TEXT_TYPE: {
        ...
        try {
          result = await MpdParsingService.validateContentByText({
            mpdText,
            activeModules,
          });
        } catch (error) {
      ...
      case FILE_TYPE: {
        ...
        try {
          result = await MpdParsingService.validateContentByFile({
            mpdFile,
            activeModules,
          });
        } catch (error) {
  ...
  function render(elementId) {
    console.log('# RENDER, state:', _state.validatorState)
      ...
      children: [
          ...
          : {},
        {
          ...
          children: [
            ...
            {
              className:
                "d-grid gap-2 d-md-flex justify-content-md-start align-items-center",
              children: [
                {
                  id: _durationElementId,
                },
